...
<center>
		...
		<th width=100 bgcolor=#113366><font color=#ffffee size=2>조회수</th>
	  	...
	  	<c:forEach items="${list}" var="row">
		    <tr bgcolor=pink>
			   <td align=center bgcolor=yellow>&nbsp;<font size=2>${row.num}</td>	<!-- row.getNum() -->
		       
		      <td align=center bgcolor=yellow>&nbsp;<font size=2>
		      	<a href="read.bod?num=${row.num}">${row.title}</a>
		      </td>
		       
		      <td align=center bgcolor=yellow>&nbsp;<font size=2>${row.name}</td>
		       
		      <td align=center bgcolor=yellow>&nbsp;<font size=2>${row.wdate}</td>
		       
		      <td align=center bgcolor=yellow>&nbsp;<font size=2>${row.count}</td>	       
		    </tr>
	    </c:forEach>
